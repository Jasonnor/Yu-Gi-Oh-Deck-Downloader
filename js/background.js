...
chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
    if (request.method == 'sendImage') {
		...
		sendResponse({data: 'Get image : ' + imageUrl + ', Name : ' + imageName});
		chrome.downloads.download({url: imageUrl, filename: 'Images\/' + imageName}, function(downloadId) {
...
function executeScript(id) {
	chrome.tabs.executeScript(id, {file: 'js/jquery-2.2.0.min.js'});
	chrome.tabs.executeScript(id, {file: 'js/deck-downloader.js'});
}
